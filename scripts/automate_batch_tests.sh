...
${compiler} ${testscript} --testid ${testid} --simSecond ${simSecond} --timeOfDay ${timeOfDay} --rain ${rain} --fog ${fog} --wetness ${wetness} --cloudiness ${cloudiness} --damage ${damage} --timeOfDay ${timeOfDay}
done
done
done
done
...
echo "${testscript}: ${testid}"
${compiler} ${testscript} --testid ${testid} --simSecond ${simSecond} --timeOfDay ${timeOfDay} --rain ${rain} --fog ${fog} --damage ${damage} --timeOfDay ${timeOfDay} --pedSpeed ${pedSpeed} --pedDistanceFromIntersection ${pedDistanceFromIntersection} --pedTrigger ${pedTrigger}
done
done
done
done
